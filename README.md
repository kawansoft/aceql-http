...
# AceQL HTTP v8.1   - October 19,  2021
...
```bash
$ wget https://download.aceql.com/rest/soft/8.1/download/aceql-http-8.1.run
```
...
```bash
chmod+x aceql-http-8.1.run                                       
./aceql-http-8.1.run
```

This will create the `aceql-http-8.1` folder that you can move where you want.

The full path to the final `aceql-http-8.1` installation folder will be surnamed **ACEQL_HOME** in following text.

Example: if you run `aceql-http-8.1.run` from `/home/mike`, then software is installed in `/home/mike/aceql-http-8.1` which is the value of **ACEQL_HOME.**
...
Add to your PATH the path to the bin directory of aceql-http-8.1 installation:

```bash
$ PATH=$PATH:/path/to/aceql-http-8.1/bin/
export PATH
```
...
```
AceQL HTTP Community v8.1 - 19-Oct-2021
```
...
<img src="https://docs.aceql.com/rest/soft/8.1/img/aceql_windows_gui_home_flatlaf.png" alt="AceQ HTTP GUI Main Windows"/> 
...
The [CsvRulesManager](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/firewall/CsvRulesManager.html) manager allows to define detailed rules just using a CSV file.
...
Here is an example of a documented CSV File: [sampledb_rules_manager.csv](http://www.aceql.com/rest/soft/8.1/src/sampledb_rules_manager.csv).
...
[aceql-server.properties](https://docs.aceql.com/rest/soft/8.1/src/aceql-server.properties). 
...
- Click [here](https://docs.aceql.com/rest/soft/8.1/src/aceqlhttp.sh) to download `aceqlhttp.sh`
...
This is done through your own implementation of the [DatabaseConfigurator](http://www.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) interface: overload the `DatabaseConfigurator.getConnection()` method in your concrete class implementation.
...
Session security is managed by implementing the [SessionConfigurator](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) interface that defines how to generate and verify the session id for (username, database) sessions. 
...
[DefaultSessionConfigurator](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/session/DefaultSessionConfigurator.html): 

- Session ids are generated using a `SecureRandom` with the [SessionIdentifierGenerator](http://www.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/session/SessionIdentifierGenerator.html) class.
...
Session management using JWT is coded in [JwtSessionConfigurator](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/session/JwtSessionConfigurator.html).
...
If you want to create your session management using your own session id generation and security rules, you can implement  the [SessionConfigurator](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) in your own class, and then: 
...
Note that in this 8.1 version SQL transactions are not supported in stateless mode.
...
The [SqlFirewallManager](http://www.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/firewall/SqlFirewallManager.html) interface allows you to code your own firewall rulesets or plug a third party software.
...
The encrypted values are generated using the [PropertiesEncryptor](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/auth/crypto/PropertiesEncryptor.html) class which is wrapped in the provided  `properties-encryptor-1.0.jar`.  The `PropertiesEncryptor` class allows both:
...
You may start or stop the AceQL Server from a Java program calling the [WebServerApi](https://docs.aceql.com/rest/soft/8.1/javadoc/org/kawanfw/sql/api/server/web/WebServerApi.html) API.
...
{
    "status": "OK",
    "version": "AceQL HTTP v8.1 - 05-Aug-2021"
}         
```
