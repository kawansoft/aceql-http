...
# AceQL HTTP v9.0   - December 2,  2021
...
```bash
$ wget https://download.aceql.com/rest/soft/9.0/download/aceql-http-9.0.run
```
...
```bash
chmod+x aceql-http-9.0.run                                       
./aceql-http-9.0.run
```

This will create the `aceql-http-9.0` folder that you can move where you want.

The full path to the final `aceql-http-9.0` installation folder will be surnamed **ACEQL_HOME** in following text.

Example: if you run `aceql-http-9.0.run` from `/home/mike`, then software is installed in `/home/mike/aceql-http-9.0` which is the value of **ACEQL_HOME.**
...
Add to your PATH the path to the bin directory of aceql-http-9.0 installation:

```bash
$ PATH=$PATH:/path/to/aceql-http-9.0/bin/
export PATH
```
...
```
AceQL HTTP Community v9.0 - 02-Dec-2021
```
...
<img src="https://docs.aceql.com/rest/soft/9.0/img/aceql_windows_gui_home_flatlaf.png" alt="AceQ HTTP GUI Main Windows"/> 
...
The [CsvRulesManager](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/firewall/CsvRulesManager.html) manager allows to define detailed rules just using a CSV file.
...
Here is an example of a documented CSV File: [sampledb_rules_manager.csv](http://www.aceql.com/rest/soft/9.0/src/sampledb_rules_manager.csv).
...
[aceql-server.properties](https://docs.aceql.com/rest/soft/9.0/src/aceql-server.properties). 
...
- Click [here](https://docs.aceql.com/rest/soft/9.0/src/aceqlhttp.sh) to download `aceqlhttp.sh`
...
This is done through your own implementation of the [DatabaseConfigurator](http://www.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) interface: overload the `DatabaseConfigurator.getConnection()` method in your concrete class implementation.
...
Session security is managed by implementing the [SessionConfigurator](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) interface that defines how to generate and verify the session id for (username, database) sessions. 
...
[DefaultSessionConfigurator](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/session/DefaultSessionConfigurator.html): 

- Session ids are generated using a `SecureRandom` with the [SessionIdentifierGenerator](http://www.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/session/SessionIdentifierGenerator.html) class.
...
Session management using JWT is coded in [JwtSessionConfigurator](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/session/JwtSessionConfigurator.html).
...
If you want to create your session management using your own session id generation and security rules, you can implement  the [SessionConfigurator](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) in your own class, and then: 
...
The [SqlFirewallManager](http://www.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/firewall/SqlFirewallManager.html) interface allows you to code your own firewall rulesets or plug a third party software.
...
The encrypted values are generated using the [PropertiesEncryptor](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/auth/crypto/PropertiesEncryptor.html)  class which allows both:
...
You may start or stop the AceQL Server from a Java program calling the [WebServerApi](https://docs.aceql.com/rest/soft/9.0/javadoc/org/kawanfw/sql/api/server/web/WebServerApi.html) API.
...
{
    "status": "OK",
    "version": "AceQL HTTP v9.0 - 05-Nov-2021"
}         
```
