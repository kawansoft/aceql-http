...
This will create the `aceql-http-8.0` folder.

The full path to the `aceql-http-8.0` installation folder will be surnamed `ACEQL_HOME` in following text.

Example: if you run `aceql-http-8.0.run` from `/home/mike`, then software is installed in

 `/home/mike/aceql-http-8.0` which is the value of `ACEQL_HOME`.
...
```bash
$ PATH=$PATH:/path/to/aceql-http-8.0/bin/;export PATH
```
...
[ACEQL HTTP START] AceQL HTTP Community v8.0 - 01-Sep-2021
