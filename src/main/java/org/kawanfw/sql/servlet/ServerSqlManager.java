...
public class ServerSqlManager extends HttpServlet {
	...
	try {
		...
		if (databaseConfiguratorClassName != null
		    ...
		    databaseConfigurator = (DatabaseConfigurator) c
			    .newInstance();
		...
		blobDownloadConfigurator = (BlobDownloadConfigurator) c
			.newInstance();
	    } else {
		...
		blobUploadConfigurator = (BlobUploadConfigurator) c
			.newInstance();
	    } else {
	    ...
	    if (!blobDownloadConfiguratorClassName
		    .equals(org.kawanfw.sql.api.server.blob.DefaultBlobDownloadConfigurator.class
			    .getName())) {
		System.out.println(SqlTag.SQL_PRODUCT_START
	    ...
	    if (!blobUploadConfiguratorClassName
		    .equals(org.kawanfw.sql.api.server.blob.DefaultBlobUploadConfigurator.class
			    .getName())) {
		System.out.println(SqlTag.SQL_PRODUCT_START
	    ...
	    if (!sessionManagerConfiguratorClassName
		    .equals(org.kawanfw.sql.api.server.session.DefaultSessionConfigurator.class
			    .getName())) {
		System.out.println(SqlTag.SQL_PRODUCT_START
	...
	if (exception != null) {
	    ...
	    JsonErrorReturn jsonErrorReturn = new JsonErrorReturn(response,
		    ...
		    JsonErrorReturn.ERROR_ACEQL_ERROR, initErrrorMesage
