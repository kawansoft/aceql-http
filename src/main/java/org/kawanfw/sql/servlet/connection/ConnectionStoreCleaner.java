...
public class ConnectionStoreCleaner extends Thread {
    ...
    /** Defines the number of milliseconds between two clean calls. One hour is good value */
     ...
     * @return true if last clean has been done in more than {@code ConnectionStoreCleaner.CLEAN_INTERVAL_MILLISECONDS}
     */
	...
	try {
		...
		ConnectionStore connectionStore = new ConnectionStore(
			connectionKey.getUsername(), connectionKey.getSessionId(),
			connectionKey.getConnectionId());
	    ...
	    for (Iterator<ConnectionKey> iterator = keysToRemove
		    .iterator(); iterator.hasNext();) {
		ConnectionKey key = (ConnectionKey) iterator.next();

		ConnectionStore connectionStore = new ConnectionStore(
			key.getUsername(), key.getSessionId(),
			key.getConnectionId());
