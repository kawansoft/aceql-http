...
public class TomcatConnectorsUpdater {
	...
	public void updateToHttp2Protocol() {
		String updateToHttp2Protocol = properties.getProperty("updateToHttp2Protocol");

		if (updateToHttp2Protocol != null && Boolean.parseBoolean(updateToHttp2Protocol)) {
			System.out.println(SqlTag.SQL_PRODUCT_START + " Protocol updated to HTTP/2");
	...
	public void setDefaultConnectorSslValues() throws DatabaseConfigurationException, ConnectException {
		...
		if (enumeration.hasMoreElements()) {
			System.out.println(SqlTag.SQL_PRODUCT_START + " Setting Default Connector SSL attributes:");
		}
	...
	private String getMandatoryPropertyValue(String propertyName) throws DatabaseConfigurationException {
		...
		if (properteyValue == null || properteyValue.isEmpty()) {
			throw new DatabaseConfigurationException("SSL activation. Property " + propertyName
					+ " name is not defined or has empty value in properties file. " + SqlTag.PLEASE_CORRECT);
		}
