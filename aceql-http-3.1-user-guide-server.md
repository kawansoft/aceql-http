# AceQL HTTP v3.1   - December 24,  2018
...
```bash
wget https://www.aceql.com/rest/soft/3.1/download/aceql-http-3.1.run
```
...
```bash
chmod+x aceql-http-3.1.run                                       
./aceql-http-3.1.run
```

This will create the `aceql-http-3.1` folder that you can move where you want.

The full path to the final `aceql-http-3.1` installation folder will be surnamed **ACEQL_HOME** in following text.

Example: if you run `aceql-http-3.1.run` from `/home/mike`, then software is installed in `/home/mike/aceql-http-3.1` which is the value of **ACEQL_HOME.**
...
Add to your PATH the path to the bin directory of aceql-http-3.1 installation:

```bash
$ PATH=$PATH:/path/to/aceql-http-3.1/bin/
export PATH
```
...
```
AceQL HTTP Community v3.1 - 24-dec-2018
```
...
<img src="https://www.aceql.com/rest/soft/3.1/img/aceql_windows_gui_home.png" alt="AceQ HTTP GUI Main Windows"/> 
...
[model-aceql-server.properties](https://www.aceql.com/rest/soft/3.1/src/model-aceql-server.properties). 
...
- Click [here](https://www.aceql.com/rest/soft/3.1/src/aceqlhttp.sh) to download `aceqlhttp.sh`
...
You may start or stop the AceQL Server from a Java program calling the [WebServerApi](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/web/WebServerApi.html) API.
...
{                                            
  "status":"OK",                           
  "version":"AceQL HTTP v3.1 – 17-dec-2018"
}         
```
...
The [DatabaseConfigurator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) interface allows you to define:
...
AceQL comes with a default `DatabaseConfigurator` implementation that may be extended: [DefaultDatabaseConfigurator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DefaultDatabaseConfigurator.html).
...
This section will illustrate how to code your own configuration methods in a concrete implementation of the [DatabaseConfigurator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) interface.
...
This is not necessary if you use the default Tomcat JDBC Pool embedded in AceQL: [DefaultDatabaseConfigurator.getConnection()](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DefaultDatabaseConfigurator.html#getConnection(java.lang.String)).
...
Please check the Javadoc of [DatabaseConfigurator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) for more information. 
...
The SQL security settings are coded in a concrete implementation of the [DatabaseConfigurator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html) interface with the following methods:
...
AceQL default implementation ([DefaultDatabaseConfigurator](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DefaultDatabaseConfigurator.html)) has minimal restrictions and should *not* be used in production code. It allows *all* SQL code execution.
...
The [StatementAnalyzer](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/StatementAnalyzer.html) utility class allows analysis of the SQL statement.
...
After server authentication succeeds (through the [DatabaseConfigurator.login()](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/DatabaseConfigurator.html#login(java.lang.String,%20char[],%20java.lang.String,%20java.lang.String)). method), the AceQL Manager builds an authentication session id that is sent back to the client and will be used by each succeeding client call in order to authenticate the calls. 

Session security is managed by implementing the [SessionConfigurator](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) interface that defines how to generate and verify the session id for (username, database) sessions. 
...
[DefaultSessionConfigurator](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/session/DefaultSessionConfigurator.html): 

- Session ids are generated using a `SecureRandom` with the [SessionIdentifierGenerator](http://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/session/SessionIdentifierGenerator.html) class.
...
Session management using JWT is coded in [JwtSessionConfigurator](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/session/JwtSessionConfigurator.html).
...
If you want to create your session management using your own session id generation and security rules, you can implement  the [SessionConfigurator](https://www.aceql.com/rest/soft/3.1/javadoc/org/kawanfw/sql/api/server/session/SessionConfigurator.html) in your own class, and then: 
